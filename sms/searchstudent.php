...
<body>
   ...
   <section>
       <div class="row">
           ...
           <div class="col-md-7 offset-md-3 text-center edit" style="margin-top:-1320px">
           <h2>Search student by Roll Number</h2><br>
            ...
            <?php
               ...
               if(isset($_POST['add'])){
                   ...
                   $edit=connect()->query("SELECT * FROM students WHERE roll_no LIKE '$roll'");
                   while($edit_id=$edit->fetch_object())
                   {
                       ?>
                    <div class="card">
                        <div class="card-body">
                            <h3 class="text-center" style="	color:#2196f3;font-weight:600">Student's Information</h3><hr><br>
                <form action="" method="POST" entype="multipart/form-data">
					<div class="form-group">
						<label for="">Roll No</label>
						<input name="roll_no" class="form-control" value="<?php echo $edit_id->roll_no; ?>" type="text" disabled>
					</div>
					<div class="form-group">
						<label for="">First Name</label>
						<input name="fname"class="form-control" value="<?php echo $edit_id->fname; ?>" type="text" disabled>
					</div>
					<div class="form-group">
						<label for="">Last Name</label>
						<input name="lname" class="form-control" value="<?php echo $edit_id->lname; ?>" type="text" disabled>
					</div>
					<div class="form-group">
						<label for="">Mother's Name</label>
						<input name="mname" class="form-control" value="<?php echo $edit_id->mother_name; ?>" type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Father's Name</label>
						<input name="faname" class="form-control"  value="<?php echo $edit_id->father_name; ?>" type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Date Of Birth</label>
						<input name="dob" class="form-control" value="<?php echo $edit_id->dob; ?>"  type="date" disabled>
					</div>
                    <div class="form-group">
						<label for="">Email</label>
						<input name="email" class="form-control" value="<?php echo $edit_id->email; ?>"  type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Password</label>
						<input name="pass" class="form-control" value="<?php echo $edit_id->password; ?>"  type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Mobile</label>
						<input name="mobile" class="form-control" value="<?php echo $edit_id->mobile; ?>" type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Class</label>
						<input name="class" class="form-control" value="<?php echo $edit_id->class; ?>" type="text" disabled>
					</div>
                    <div class="form-group">
						<label for="">Address</label>
						<input name="address" class="form-control" value="<?php echo $edit_id->address; ?>" type="text" disabled>
					</div>
